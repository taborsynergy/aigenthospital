...
def send_whatsapp(to: str, body: str, from_: Optional[str] = None) -> bool:
    """Send WhatsApp message via Twilio. Requires from_ to be 'whatsapp:+1234567890' format."""
    if not to or not to.strip():
        return False

    sender = from_ or f"whatsapp:{settings.twilio_default_number}"
    client = _client()

    if not client:
        logger.info("[WHATSAPP-MOCK] To=%s From=%s | %s", to, sender, body[:120])
        return True
    ...
    try:
        ...
        logger.error("Twilio WhatsApp error to=%s: %s", to, exc)
        return False
...
def send_whatsapp_appointment_reminder(
    ...
    if clinic_phone:
        body += f" Questions? Call {clinic_phone}."
    return send_whatsapp(to, body, from_)


def send_whatsapp_recall(
    to: str,
    patient_name: str,
    visit_type: str,
    clinic_name: str,
    ...
    body = (
        f"Hi {patient_name}! 👋 It's been a while since your last visit at {clinic_name}. "
        f"You may be due for your {visit_type}. "
        f"Reply 📅 to schedule or STOP to unsubscribe."
    )
    return send_whatsapp(to, body, from_)
